...
`su user` - переключить пользователя  
...
превратить сервеную систему в десктоп
`sudo apt-get update`\
```
sudo apt-get install tasksel
sudo tasksel # оболочка для установки различных пакетов
```
`sudo apt-get install ubuntu-desktop` - или собственно сам desctop
...
`sudo ip link set enp0s3 up` - включить интерейс
'sudo dhclient enp0s3' - получить параметры по DHCP

Netplan должен быть всегда, networkmanager - в десктопных версиях.

`ss -tnlp` - посмотреть открытые порты
