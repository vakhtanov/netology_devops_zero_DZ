...
`locate -е NAME` - найти файл в базе и проверить его существование\
`locate --regex "(\.jpg|\.mp3)$"` - найти по регулярным выражениям\
`locate -i NAME` - игнорировать регистр
...
## vim (новый)/ vi (старый)
два режима - коммандный и редактирвоания\
на клавиатуре\
`i` - перейти в ражим вставки\
`esc` - выйти из режима вставки\
`dd` - удалить строку\
`u` - отменить последнее действие\
`:wq` - выйти из редактора с сохранием\
`a` - перейти в режим вставки после курсова\
`q!` - выйти без сохранения\
`:w FILE` - записать изменения в файл\
`o` - Вставить строку после текущей\
`O` - Вставить строку над текущей\
`ZZ` - Выйти с сохранением файла\
`ZQ` - Выйти без сохранения\
`/TEXT` - поиск текста\
`:colorscheme <TAB> darkblue` - поменять цветовую схему\

## nano
`^` - символ обозначает `ctrl`\
`ctrl+g` - подсказка, `ctrl+x` - закрыть подсказка\
основные комманды перечислены внизу экрана\
`ctrl+o` - сохранить\
`nano -ET4 FILE` - замена табуляции на 4 пробела\
`nano -P FILE` - запоминать положение курсора\
`ctrl+w` - поиск\
`ctrl+\` - замена\
`alt+u` - отменить последенее действие\
`alt+e` - повторить последенее действие\
